...
def detect_faces(img) :
	gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)#TO CONVERT TO GREY
	faceCasc = cv2.CascadeClassifier("haarcascade_frontalface_default.xml")#CLASSIFIER NAME
	faces = faceCasc.detectMultiScale(gray, 1.3, 5)##DETECT FACES IN IMAGE,USING MULTISCALE,1.3HOW MUCH IMAGE SIZE IS REDUCED IN EACH SCLE,5 IS NUMBER OF NEIGHBOUR EACH RECTANGLE SHOULD HAVE TO RETAIN IT
	graylist = []
	faceslist = []
	...
	for i in range(0, len(faces)) :
		(x, y, w, h) = faces[i]#VALUE OF X,Y,WIDTH AND HEIGHT AND ALL THESE ASPECTS ARE FOR FACES IN THIS
